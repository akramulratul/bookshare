...
import { updatedIsShare, deleteAShareBook } from "../../../../actions/shareBooks";
...
const Book = ({ book }) => {
  ...
  const getBook = () => {
    history.push(`/allshare/book/${book._id}`);
  };
  ...
  const deleteBook = () => {
    dispatch(deleteAShareBook(book._id));
  };
  ...
  const soldBook = () => {
    dispatch(updatedIsShare(book._id));
  };
