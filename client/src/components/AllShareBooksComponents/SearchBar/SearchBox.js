...
const SearchBox = () => {
  ...
  const allbooks = useSelector((state) => state.shareBooks);
  const books = allbooks.filter((book) => book.isShare === false);
  ...
  const updateBooks = () => {
    if (
      ...
      for (const book of books) {
        ...
        if (inputLocation !== "") {
          if (
            book.location.toLowerCase().includes(inputLocation.toLowerCase())
          ) {
            if (filteredBookArr.length !== 0) {
              if (filteredBookArr[filteredBookArr.length - 1]._id !== book._id)
                filteredBookArr.push(book);
