...
import Error404 from "../../ErrorComponent/Error404.js";
import { GET_SHARE_BOOK } from "../../../constants/actions.js";
import Carousel from "react-multi-carousel";
import "react-multi-carousel/lib/styles.css";
import Book from "../Book/Book.js";

const ShareBookInfo = ({ match }) => {
  const classes = useStyles();
  const history = useHistory();
  const dispatch = useDispatch();
  const books = useSelector((state) => state.shareBooks);
  const book = useSelector((state) => state.shareBook);
              ...
              <div>
                <ul style={{ listStyleType: "none" }} className={classes.list}>
                  ...
                  <li>
                    Direction:{" "}
                    <span className={classes.name}>
                      <a href={book.direction} target="_blank">
                        Location Link
                      </a>
                    </span>
                  </li>
