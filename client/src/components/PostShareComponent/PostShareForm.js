...
const PostShareForm = () => {
  ...
  const [shareBookData, setShareBookData] = useState(initialState);
  const [user, setUser] = useState(JSON.parse(localStorage.getItem("profile")));
  const [err, setErr] = useState(false);
  const ShareBook = useSelector((state) => state.ShareBook);
  ...
  const addImage = async (e) => {
    ...
    setShareBookData({ ...shareBookData, selectedFile: imageFile });
  };
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={12}>
                    <TextField
                      ...
                      multiline
                      rows={4}
                      value={shareBookData.description}
                      onChange={handleChange}
                    />
                  </Grid>
                </Grid>
